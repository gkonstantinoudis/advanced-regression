...
lapply(res_store, unlist) %>% do.call(rbind, .) %>% as_tibble() %>% mutate(type = 
                                                                             factor(paste0("lag ", 0:15), 
                                                                                    levels = paste0("lag ", 0:15))) -> plotres
...
ggplot(data = plotres) + 
  ...
  geom_hline(yintercept = 0, col = "red", linetype = "dotted") + theme_bw() + ylab("log risk PM10") + xlab("Lags") -> p1
...
ggplot(data = plotres) + 
  ...
  geom_hline(yintercept = 0, col = "red", linetype = "dotted") + theme_bw() + ylab("log risk PM10") + xlab("Lags") -> p2
...
library(patchwork)
(p1 + theme(text = element_text(size = 15)))/(p2 + theme(text = element_text(size = 15)))
```
...
-   The \<span style:"color:red"\> overall impact </span> for a unit change in $X$ is given by $\sum^q_{\ell=0}\beta_\ell$.
...
#| eval: true
#| fig-align: "center"
plot(pred1.pm, ptype = "slices", var = 1,  
