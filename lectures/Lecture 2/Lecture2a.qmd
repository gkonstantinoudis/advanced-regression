...
ggplot(data = data.frame(x=x,y=y), aes(x=x, y=y)) + geom_point(cex = 1.2) + 
  theme_bw() + geom_smooth(method='lm', formula= y~x, se = FALSE, linewidth = 1) + 
...
ggplot() + geom_hline(yintercept = leg4coef[[1]], linewidth = 1) + xlim(-1, 1) + ylim(-1, 2) + theme_bw() + ylab("") + xlab("") -> p1 
chicagoNMMAPS %>% filter(year == 1987) %>% 
  ggplot(aes(x=time, y=temp)) + geom_point(cex = 1.3) + theme_bw() + 
...
ggplot() + 
  ...
  xlim(-1, 1) + ylim(-1, 2) + theme_bw() + ylab("") + xlab("") -> p1 
...
ggplot() + 
  ...
  xlim(-1, 1) + ylim(-1, 2) + 
  geom_line(aes(x = seq(from = -1, to = 1, length.out = 1000), 
                y = -0.5 + 1.5 * seq(from = -1, to = 1, length.out = 1000)^2), linewidth = 1, col = cols[2]) + theme_bw() + ylab("") + xlab("") -> p1
...
ggplot() + 
  ...
  xlim(-1, 1) + ylim(-1, 2) + 
  geom_line(aes(x = seq(from = -1, to = 1, length.out = 1000), 
                y = -0.5 + 1.5 * seq(from = -1, to = 1, length.out = 1000)^2), col = cols[2], cex = .6, linewidth = 1) + 
  geom_line(aes(x = x.plot, 
                y = y), col = cols[3], cex = .6, linewidth = 1) + theme_bw() + ylab("") + xlab("") -> p1 
...
$$Y _i = \beta_0 + \beta_1 X + \beta_2 X^2 + \beta_3 X^3 + \beta_4 X^4 + \epsilon$$
...
ggplot() + 
  ...
  geom_line(aes(x = seq(from = -1, to = 1, length.out = 1000), 
                y = -0.5 + 1.5 * seq(from = -1, to = 1, length.out = 1000)^2), col = cols[2], cex = .6, linewidth = 1) + 
  geom_line(aes(x = x.plot, 
                y = y), col = cols[3], cex = .6, linewidth = 1) + 
  geom_line(aes(x = x.plot.1, 
                y = y.1), col = cols[4], cex = .6, linewidth = 1) + theme_bw() + ylab("") + xlab("") -> p1 
...
ggplot() + 
  ...
  geom_line(aes(x = x.plot, 
                y = y), col = cols[3], cex = .6, linewidth = 1) + 
  geom_line(aes(x = x.plot.1, 
                y = y.1), col = cols[4], cex = .6, linewidth = 1) + 
  geom_line(aes(x = x.plot.2, 
                y = y.2), col = cols[5], cex = .6, linewidth = 1) + theme_bw() + ylab("") + xlab("")-> p1 
...
ggplot(data = data.frame(x=x,y=y), aes(x=x, y=y)) + geom_point(cex = 3) + 
  theme_bw() + geom_smooth(method='lm', formula= y~x, se = FALSE, linewidth = 2) + 
  theme(text = element_text(size = 15))
```
...
ggplot() + 
  ...
  geom_line(aes(x = x, y = cos(2*omega*x)), cex = .6, col = cols[2]) + theme_bw() + 
  xlab("") + ylab("") -> p1
  
chicagoNMMAPS %>% filter(year == 1987) %>% 
  ggplot(aes(x=time, y=temp)) + geom_point(cex = .8) + theme_bw() + 
...
ggplot() + 
  geom_point(aes(x = x, y = y), cex = 1.3, col = cols[1]) + theme_bw() + 
...
dat %>% mutate(
  ...
  disc = case_when(dat$x<=-1~1, 
                   dat$x>-1 & dat$x<=0.5~2, 
                   dat$x>0.5 & dat$x<=1.5~3, 
                   dat$x>1.5 & dat$x<=3~4, 
                   dat$x>3~5)
) %>% 
  ggplot() + geom_point(aes(x = x, y = y), cex = 1.3, col = cols[1]) + 
...
ggplot() + 
  geom_point(aes(x = x, y = y), col = cols[1], cex = 1) + theme_bw() + 
  xlab("") + ylab("") +
...
$$
\begin{align}
Y_i &= \beta_0^{(1)} + \beta_1^{(1)}x + \epsilon \qquad x\leq-1\\
Y_i &= \beta_0^{(2)} + \beta_1^{(2)}x + \epsilon \qquad -1<x\leq0.5\\
Y_i &= \beta_0^{(3)} + \beta_1^{(3)}x + \epsilon \qquad 0.5<x\leq1.5\\
Y_i &= \beta_0^{(4)} + \beta_1^{(4)}x + \epsilon \qquad 1.5<x\leq3\\
Y_i &= \beta_0^{(5)} + \beta_1^{(5)}x + \epsilon \qquad x\geq3\\
\end{align}
$$

Linear spline model

$Y_i = \beta_0 + \beta_1x + \beta_2(x+1)_+ + \beta_3(x-0.5)_+ + \beta_4 (x-1.5)_+ + \beta_5(x-3)_+ + \epsilon$
...
$\begin{align} Y_i = \beta_0 &+ \beta_1 x + \beta_2x^2 + \beta_3x^3 + \\&\beta_4 (x+1)_+ +\beta_5 + (x+1)_+^2 +\beta_6 (x+1)_+^3 +\\ &\beta_7 (x-0.5)_+ +\beta_8 (x-0.5)_+^2 +\beta_9 (x-0.5)_+^3 + \\ & \dots + \epsilon \\(x-k)_+& = \begin{cases} 0, \quad x<k\\x-k, \quad x\geq k\end{cases} \end{align}$
...
$Y_i = \begin{align}\beta_0 &+ \beta_1 x + \beta_2x^2_+ + \beta_3x^3 + \\&+\beta_6 (x+1)_+^3 + \beta_9 (x-0.5)_+^3 + \dots + \epsilon \\\end{align}$
