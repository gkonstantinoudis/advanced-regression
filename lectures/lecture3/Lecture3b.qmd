...
ggplot() +
  geom_abline(intercept = coefs[1], slope = coefs[2], lwd = 1, col = "red") +
  # geom_segment(aes(xend = x1, yend = fitted)) +
  geom_point(data = dat, aes(x = x1, y = y1, pch = cv, col = cv), size = 3) + 
  theme_bw() +
...
ggplot() +
  geom_abline(intercept = coefs[1], slope = coefs[2], lwd = 1, col = "red") +
  geom_point(data = dat %>% dplyr::filter(cv %in% "Training data"), 
...
ggplot() + 
  ...
  annotate(geom="text", x=0.5, y=-1, label="(0,0)",
           color="black") + 
  theme_bw() + 
  ylim(c(-5, 5)) + 
  xlim(c(-5, 5)) + 
...
lasso.out = glmnet(x=as.matrix(x), y=severity, family='binomial', alpha = 1, lambda = 0.12)
sum(abs(lasso.out$beta)>0)
lasso.out$a0
...
enet.out = glmnet(x=as.matrix(x), y=severity, family='binomial', alpha = 0.5, lambda = 0.26)
sum(abs(enet.out$beta)>0)
enet.out$a0
