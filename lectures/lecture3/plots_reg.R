...
ggplot() + 
  ...
  geom_segment(aes(x = 2.5, y = 5.7, xend = 4.5, yend = 5.7),
               arrow = arrow(length = unit(0.2, "cm"))) + 
  annotate(geom="text", x=5, y=5.7, label=expression(hat(beta)[OLS]),
           color="black") + 
  annotate(geom="text", x=2.5, y=2.5, label=expression(hat(beta)[lasso]),
           color="black") + 
  annotate(geom="text", x=-0.5, y=10, label=expression(beta[1]),
           color="black") + 
  annotate(geom="text", x=7, y=-.5, label=expression(beta[2]),
           color="black") + 
  annotate(geom="text", x=2, y=-2, label=expression(L[1]~": |"~beta[1]~"|"~+~"|"~beta[2]~"| <"~lambda),
           color="black") + 
...
ggplot() + 
  ...
  annotate(geom="text", x=5, y=5.7, label=expression(hat(beta)[OLS]),
           color="black") + 
  annotate(geom="text", x=3.5, y=2.45, label=expression(hat(beta)[ridge]),
           color="black") + 
  annotate(geom="text", x=-0.5, y=10, label=expression(beta[1]),
           color="black") + 
  annotate(geom="text", x=7, y=-.5, label=expression(beta[2]),
           color="black") + 
  annotate(geom="text", x=2, y=-2, label=expression(L[2]~": "~beta[1]^2~+~beta[2]^2~" <"~lambda),
           color="black") + 
  annotate(geom="text", x=-.6, y=1.3, label=expression(sqrt(lambda)),
           color="black") +
  geom_segment(aes(x = 0, y = -3, xend = 0, yend = 10.8),
               arrow = arrow(length = unit(0.2, "cm"))) + 
  geom_segment(aes(x = -3, y = 0, xend = 8, yend = 0),
...
ggplot() + 
  ...
  geom_segment(aes(x = 2.2, y = .3, xend = 3.5, yend = .3),
               arrow = arrow(length = unit(0.2, "cm"))) +
  annotate(geom="text", x= 5.12, y=1.85, label="Ridge penalty (a=1)",
           color="black") + 
  annotate(geom="text", x=5.6, y=1, label="Elastic net penalty (a=0.5)",
           color="black") +
  annotate(geom="text", x=5.2, y=.35, label="Lasso penalty (a=0)",
           color="black") +
  annotate(geom="text", x=-0.5, y=10, label=expression(beta[1]),
           color="black") + 
  annotate(geom="text", x=7, y=-.5, label=expression(beta[2]),
