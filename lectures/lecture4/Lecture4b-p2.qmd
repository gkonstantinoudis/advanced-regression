...
![](assets/bagging_diagram.png){width=80%, fig-align="center"}
...
for (i in 1:20) {
  sample_d <- BostonHousing[sample(1:nrow(BostonHousing), nrow(BostonHousing), replace = TRUE), ]
...
p_data_boot <-
  ...
  geom_line(aes(x = log(crim), y = pred, colour = sample), data = fits, linewidth = .75) +
  labs(x = "crime score (log)", y = "median house price") +
  theme_minimal() +
...
p_data_boot_summary <-
  ggplot() +
  geom_point(aes(x = log(crim), y = medv), alpha = 0.2, size = 1, data = BostonHousing) +
  geom_line(aes(x = log(crim), y = pred), colour = "red", data = fits_summary, linewidth = .75) +
  labs(x = "crime score (log)", y = "median house price") +
  theme_minimal() +
  theme(legend.position = "none")
...
1. Output as the final model the boosted predictions
$$
...
- Ensemble methods like bagging, random forest and boosting,
fit many trees in different variations and average over them.
