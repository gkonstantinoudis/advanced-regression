...
- **Nodes** or **splits**: Points along the tree where the predictor
space is split.
...
- $R_1$ to $R_5$ map out a
partition.
- $t_1$ to $t_4$ are the split
values.
:::
:::
...
p <- ggparty(py) +
  ...
  geom_node_label(aes(label = info, col = factor(level)), ids = "terminal", size = 12) +
  theme(legend.position = "none")
...
p + inset_element(p_tree, left = 0.0, bottom = 0.6, right = 0.3, top = 1.0, clip = FALSE)
```
...
p + inset_element(p_tree, left = 0.0, bottom = 0.5, right = 0.3, top = 1.0, clip = FALSE)
```
