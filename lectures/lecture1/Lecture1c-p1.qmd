...
library(tidyverse)
data_chol <- read_csv("data-1c/cholesterol.csv")
```
...
1. Calculate summary measures for each group and use standard analysis (group-level analysis).
1. Fixed effects model to account for group structures.
1. Use random effects models that explicitly model the similarity of observations in a group.
...
Assume we have independent observations $y_k$ with variance $\sigma_k$.
Then the IVW estimate is defined as
...
Assume $y_k$ is a vector of group summaries, $x_k$ is a $k \times p$ matrix of group summaries.
Assume $w$ is a diagonal matrix with $w[k,k] = \frac{1}{\sigma_k^2}$, then the **weighted least squares** estimate is defined as
