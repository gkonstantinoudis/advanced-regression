...
ggplot(data=data.frame(x=x1, y=y1), aes(x=x1, y=y1)) + 
  ...
  theme_bw() + ylab("y-axis") + xlab("x-axis") + 
...
data.frame(
  ...
  type = c(rep("bmi", times = length(x$ldl)), 
           rep("hdl", times = length(x$hdl)))
) %>% 
  ggplot() + geom_point(aes(x=Predictors, y=Score)) + facet_grid(cols = vars(type)) + 
  theme_bw()
```
...
(ggplot(data = mod, aes(qqnorm(.stdresid, plot.it=FALSE)[[1]], .stdresid))+
  ...
  (
    ggplot() + geom_histogram(aes(x=residuals(mod))) + 
      theme_bw() + 
