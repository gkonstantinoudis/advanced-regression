...
install.packages(c(
  "dplyr", "ggplot2", "patchwork", "splines", "blmeco", "mgcv", "dlnm",
  "plotly"
))
```
```{r}
...
data.frame(
  x = rep(triceps$age, times = 3),
  fitted = c(
    mod_po2$fitted.values, mod_po3$fitted.values,
  ...
  ggplot() +
  geom_point(data = triceps, aes(x = age, y = triceps), cex = 1.2) +
...
data.frame(
  ...
  ggplot() +
  geom_point(data = triceps, aes(x = age, y = triceps), cex = .7, alpha = 0.2) +
...
data.frame(
  x = rep(triceps$age, times = 2),
  b = c(b1, b2),
  basis = rep(c("basis 1", "basis 2"), each = triceps$age %>% length())
) %>%
  ggplot() +
  geom_line(aes(x = x, y = b), cex = .8) +
...
lapply(c(1, 10, 50, 100), function(X) {
  ggplot() +
    ...
    geom_line(aes(x = triceps$age, y = lm(triceps ~ ns(age, df = X),
      data = triceps
    )$fitted.values), cex = 1.2) +
    theme_bw() +
    ggtitle(paste("df=", X, sep = " ")) %>% return()
}) -> p_ns
...
ggplot() +
  ...
  geom_ribbon(aes(ymin = LL, ymax = UL, x = triceps$age), fill = "red", alpha = 0.2) +
  theme_bw()
```
...
for (i in 1:length(k)) {
  if (i == 1) {
    chicagoNMMAPS %>%
      dplyr::mutate(
        heatwave_lagged = dplyr::case_when(
          heatwave == 1 |
  ...
  res_store[[i]] <- list(
    est = (exp(coef(tmp)["factor(heatwave_lagged)1"]) - 1) * 100,
...
ggplot(data = plotres) +
  ...
  theme_bw() +
  ylab("% increase in respiratory mortality") +
  xlab("Lags") -> p1
...
```{r message=FALSE, warning=FALSE}
cb.heatwave <- crossbasis(chicagoNMMAPS$temp,
  lag = 10,
