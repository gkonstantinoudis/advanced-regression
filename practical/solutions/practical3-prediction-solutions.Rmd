...
```{r message=FALSE, warning=FALSE}
predfun.glmnet = function(train.x, train.y, test.x, test.y, 
...
cv.out.ridge =  crossval(predfun.glmnet, x, y, 
                         lambda=ridge.cv$lambda.1se, alpha=0,
                         K=5, B=20, verbose=FALSE)
cv.out.lasso =  crossval(predfun.glmnet, x, y, 
                         lambda=lasso.cv$lambda.1se, alpha=1,
                         K=5, B=20, verbose=FALSE)
...
table.out = rbind(c(cv.out.ridge$stat, cv.out.ridge$stat.se),
 c(cv.out.lasso$stat, cv.out.lasso$stat.se), 
 c(cv.out.enet$stat, cv.out.enet$stat.se))
...
for(i in 1:ncol(x_mat)){
	results.uni.glm[i,] = summary(glm(y_bin~x_mat[,i], 
...
```{r message=FALSE, warning=FALSE}
results.uni.glm = cbind(results.uni.glm, p.adjust(results.uni.glm[,4], 
                                                  method="bonferroni"),
...
glm.out = glm(y_bin~age+duration+us_popularity_estimate+acousticness+
                ...
                instrumentalness+liveness+loudness+mechanism+speechiness+tempo+
                valence, family = "binomial")
```
...
lasso.cv = cv.glmnet(x_mat,y_bin,family="binomial",alpha=1, 
                     type.measure="deviance")
lasso.cv$lambda.1se
lasso.out.1se = glmnet(x_mat,y_bin,family="binomial",alpha=1,
...
a = seq(0.05, 0.95, 0.05)
search = foreach(i = a, .combine = rbind)%do%{
  			cv = cv.glmnet(x_mat,y_bin,family = "binomial", 
...
elasticnet.cv = search[search$cvm == min(search$cvm), ]
elasticnet.cv
enet.out = glmnet(x_mat, y_bin, family = "binomial", 
